...
int main(void)
{
    ...
    tick(true,80,0);
    assert(fabsf(mean_motor()-81.0f)<0.03f);
    /* Attitude correction remains active after an immediate throttle step. */
    tick(true,80,10);
    ...
    assert(fabsf(mean_motor()-81.0f)<0.03f);
    tick(true,100,0);
    assert(fabsf(mean_motor()-100.0f)<0.03f);
    puts("Direct throttle, PID authority and disarm tests passed");
}
